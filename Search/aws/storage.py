...
class S3Storage(Storage):
    def __init__(self):
        assert hasattr(settings, 'AWS_S3_BUCKET_NAME'), 'AWS_STORAGE_BUCKET_NAME is not set '
    ...
    def url(self, name):
        return self.client.generate_presigned_url(
            'get_object', Params={'Bucket': self.bucket_name, 'Key': name}, ExpiresIn=self.expiration_time
        )
